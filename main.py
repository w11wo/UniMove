...
model = Traj_Model(Traj_Config(n_embd=args.n_embd,n_head=args.n_head,n_layer=args.n_layer,num_experts=args.num_experts,top_k=args.top_k)).to(args.device)
...
train_stop(model,train_loader=train_loader,valid_loaders=val_loader,log_dir=log_dir,lr=args.lr,epoch=args.epoch,valid_step_interval=valid_step_interval,device = args.device,citys = args.target_city,patience=10)
